...
export function AdminWorkspace({ section, summary = [{ label: "Restricted accounts", value: "0" }, { label: "Security events", value: "0" }, { label: "Pending role changes", value: "0" }, { label: "System status", value: "Healthy" }], data, actions }: Readonly<{ section: string; summary?: WorkspaceSummary; data?: WorkspaceSectionData; actions?: React.ReactNode }>) {
  return <WorkspaceScaffold title="Platform administration" eyebrow="Restricted administration" rootPath="/admin" activeSection={section} sections={ADMIN_SECTIONS} summary={summary}><WorkspaceRecords data={data}>{actions}</WorkspaceRecords></WorkspaceScaffold>;
}
